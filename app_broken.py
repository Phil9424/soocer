...
def calculate_lineup_strength(lineup):
    """Вычисляет силу состава на основе рейтингов игроков"""
    if not lineup:
        return 70  # Средний рейтинг по умолчанию

    total_rating = sum(player.get('rating', 70) for player in lineup)
    return total_rating / len(lineup)

# Функция для выбора бомбардира из состава пользователя
def select_goal_scorer(game_data, lineup, match_goals=None):
    """Выбирает бомбардира из состава пользователя"""
    import random
    
    if not lineup:
        return 'Неизвестный игрок'
    
    # Выбираем случайного игрока из линии нападения
    forwards = [p['name'] for p in lineup if get_player_position(game_data.get('team_name', ''), lineup.index(p)) == 'FWD']
    if forwards:
        return random.choice(forwards)
    
    # Если нет форвардов, выбираем любого полевого игрока
    field_players = [p['name'] for p in lineup if get_player_position(game_data.get('team_name', ''), lineup.index(p)) != 'GK']
    if field_players:
        return random.choice(field_players)
    
    # В крайнем случае любого
    return random.choice([p['name'] for p in lineup])
def select_opponent_goal_scorer(team_name, lineup, match_goals=None):
    """Выбирает бомбардира соперника"""
    import random
    ...
    # Выбираем случайного игрока из линии нападения
    forwards = [p['name'] for p in lineup if get_player_position(team_name, lineup.index(p)) == 'FWD']
    if forwards:
        return random.choice(forwards)
    
    # Если нет форвардов, выбираем любого полевого игрока
    field_players = [p['name'] for p in lineup if get_player_position(team_name, lineup.index(p)) != 'GK']
    if field_players:
        return random.choice(field_players)
...
def match():
    try:
        if 'game_data' not in session:
            print("DEBUG: game_data not in session, redirecting to index")
            return redirect(url_for('index'))

        print(f"DEBUG: game_data found in session: {session['game_data'].keys()}")

        import random
        game_data = session['game_data']

        my_team = game_data.get('team_name')
        if not my_team:
            print("ERROR: team_name not found in game_data")
            return "Internal Server Error: Missing team_name", 500

        opponent_team = game_data.get('next_opponent')
        if not opponent_team:
            print("ERROR: next_opponent not found in game_data")
            return "Internal Server Error: Missing next_opponent", 500

        print(f"DEBUG: my_team={my_team}, opponent_team={opponent_team}")

        # Получаем составы
        my_squad = game_data.get('squad', [])
        if not my_squad:
            print("ERROR: squad not found in game_data")
            return "Internal Server Error: Missing squad", 500
        ...
        try:
            ...
            print(f"ERROR creating opponent lineup: {lineup_error}")
            opponent_lineup = opponent_squad[:11]  # Fallback
