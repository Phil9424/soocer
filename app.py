...
def game_page(page):
    ...
    # Синхронизируем current_round между session и game_data
    if 'current_round' in session:
        game_data['current_round'] = session['current_round']
    elif 'current_round' in game_data:
        session['current_round'] = game_data['current_round']
    else:
        game_data['current_round'] = 1
        session['current_round'] = 1
    
    # Сохраняем синхронизированный game_data обратно в сессию
    session['game_data'] = game_data
